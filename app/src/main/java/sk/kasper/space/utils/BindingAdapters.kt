...
@BindingAdapter(
        ...
        requireAll = true)
fun setLaunchTimeFormat(view: TextView, launchDateTime: LocalDateTime, prettyTimeVisible: Boolean, formattedTimeVisible: Boolean, formattedTimeType: FormattedTimeType, dateConfirmed: Boolean) {
    if (!dateConfirmed) {
...
fun setTint(imageView: ImageView, @DrawableRes drawableRes: Int, @ColorRes tintColor: Int) {
    ...
    imageView.setImageDrawable(getVectorDrawableWithTint(context, drawableRes, ContextCompat.getColor(context, tintColor)))
}

@BindingAdapter(value = ["tint"])
fun setTint(imageView: ImageView, @ColorRes tintColor: Int) {
    if (tintColor != 0) {
        imageView.setImageDrawable(setTint(imageView.drawable, ContextCompat.getColor(imageView.context, tintColor)))
    }
}
...
@BindingAdapter(
        ...
        requireAll = true)
fun setCircleImage(imageView: ImageView, @DrawableRes circleResource: Int, fallbackName: String, roundedSquareLetterProvider: RoundedSquareLetterProvider) {
    if (circleResource == 0) {
        imageView.setImageBitmap(roundedSquareLetterProvider.createLetter(
                fallbackName.first()))
    } else {
        Picasso.with(imageView.context)
                .load(circleResource)
                .transform(RoundedSquareTransformation(R.dimen.rounded_corners_radius.toPixels(imageView.context).toFloat()))
...
fun setBackgroundTint(view: View, @ColorRes colorRes: Int) {
    view.backgroundTintList = if (colorRes != 0) ColorStateList.valueOf(ContextCompat.getColor(view.context, colorRes)) else null
}
...
fun getVectorDrawableWithTint(context: Context, @DrawableRes resourceId: Int, @ColorInt color: Color): Drawable? {
